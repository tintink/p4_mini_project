...
(Note: Best way to read blog is either through this Hackmd link: or the HTML file in submission folder. 
...
(Caveat: The general mortality rate is a good general indication of the risk of dying from Covid-19 among the population and is adequate for this analysis. However, it is limited as not everyone within the population is exposed to the same uniform risk of infection. The Infection fatality ratio is a better measure that represents the proportion of deaths among infected individuals but is difficult to calculate accurately and requires detailed data on infected persons that is not readily available)
...
Looking at the absolute numbers of deaths of the two viruses also helps to further dispel some of the fears many people have about the risk they have of dying from the virus. In the whole of 2020, only 35 people died with Covid-19 in the 20-24 category where the majority of the student population lies, a relatively lower number. 


### 2.) Indirect impact on mortality 


Not only has the virus been deadly through the direct deaths it's caused as seen in section 1, the indirect impacts of lockdown and social distancing restrictions have also been considerable. Whilst these measures were initially implemented to protect lives by stopping the spread of the virus they also made it increasingly difficult for doctors to see and treat patients for other conditions. The hysteria caused by reasons described earlier and the unknown nature of a pandemic also discouraged people from going to the hospital. These indirect effects of the virus could prove even more deadly than the direct deaths the virus has caused so far. To analyze this I took the number of new referrals made by doctors in England and Wales in 2020 and compared them to average levels in 2019 before the pandemic struck.
...
```{r fig.height=6, fig.width=10, message=FALSE}
ggsave("p4_plots/covid_mortality_rates_plot.png", covid_mortality_rates_plot,width = 10, height = 6, dpi=100)
ggsave("p4_plots/mortality_rate_comparison_plot.png", mortality_rate_comparison_plot,width = 10, height = 6, dpi=100)
ggsave("p4_plots/total_deaths_comparison_plot.png", total_deaths_comparison_plot,width = 10, height = 6, dpi=100)
ggsave("p4_plots/new_rtt_period_plot.png", new_rtt_period_plot,width = 10, height = 6, dpi=100)
ggsave("p4_plots/total_deaths_plot_G7.png", total_deaths_plot_G7,width = 10, height = 6, dpi=100)
```
```
